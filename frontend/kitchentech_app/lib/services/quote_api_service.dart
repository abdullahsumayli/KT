...
class QuoteApiService {
  ...
  static Future<Map<String, dynamic>> submitQuoteRequest({
    ...
    try {
      // تحضير البيانات
      final requestData = {'style': style, 'city': city, 'phone': phone};
      ...
      if (response.statusCode == 201) {
        ...
        // خطأ مصادقة - لكن هذا endpoint عام ولا يحتاج مصادقة!
        // قد تكون المشكلة في إعدادات السيرفر
        final errorData = jsonDecode(response.body);
        final errorMessage = errorData['detail'] ?? 'خطأ في الوصول للخدمة';
        throw QuoteApiException(
          'خطأ في المصادقة: $errorMessage\nالرجاء التواصل مع الدعم الفني',
        ...
        // طلب مكرر
        throw QuoteApiException(
          'تم إرسال طلب من هذا الرقم مسبقاً خلال آخر 24 ساعة',
        ...
        throw QuoteApiException(errorMessage, errorCode: 'VALIDATION_ERROR', statusCode: 400);
        ...
        throw QuoteApiException(
          'حدث خطأ غير متوقع (${response.statusCode}): $errorBody',
...
class QuoteApiException implements Exception {
  ...
  QuoteApiException(this.message, {required this.errorCode, this.statusCode, this.originalError});
  ...
  String get userFriendlyMessage {
    switch (errorCode) {
      ...
      case 'AUTH_ERROR':
        return message; // استخدم الرسالة التفصيلية
      case 'DUPLICATE_REQUEST':
        return 'تم إرسال طلب من هذا الرقم مسبقاً خلال آخر 24 ساعة.\nللاستفسار اتصل بنا مباشرة';
      case 'RATE_LIMIT':
